...
<div class="d-flex align-items-center justify-content-between mb-4">
    ...
    <a href="{{ route('plataforma.create') }}" class="btn btn-gv"><i class="bi bi-plus-circle me-2"></i> Nova Plataforma</a>
</div>
...
<div class="card-gv mb-3">
    <div style="padding:1rem 1.25rem">
        <form action="{{ route('plataforma.search') }}" method="POST" class="d-flex gap-2">
            ...
            <a href="{{ route('plataforma.index') }}" class="btn btn-outline-gv"><i class="bi bi-x"></i></a>
        </form>
    </div>
</div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        @else
                            <div style="width:40px;height:40px;background:var(--gv-border);border-radius:6px;display:flex;align-items:center;justify-content:center;color:var(--gv-muted)"><i class="bi bi-device-ssd"></i></div>
                        @endif
                    </td>
                    ...
                    <td>
                        <a href="{{ route('plataforma.edit', $item->id) }}" class="btn btn-sm btn-outline-gv"><i class="bi bi-pencil"></i></a>
                    </td>
                    <td>
                        <form action="{{ route('plataforma.destroy', $item->id) }}" method="POST">
                            ...
                            <button type="submit" class="btn btn-sm" style="border:1px solid var(--gv-red);color:var(--gv-red);background:none"
                                onclick="return confirm('Remover plataforma?')"><i class="bi bi-trash"></i></button>
                        </form>
                    </td>
                </tr>
                @empty
                <tr><td colspan="9" class="text-center" style="padding:3rem;color:var(--gv-muted)">Nenhuma plataforma cadastrada.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
