...
<div class="d-flex align-items-center gap-3 mb-4">
    <a href="{{ route('plataforma.index') }}" class="btn btn-sm btn-outline-gv"><i class="bi bi-arrow-left"></i></a>
    <h4 style="font-family:'Rajdhani',sans-serif;font-weight:700;margin:0">{{ isset($dado) ? 'Editar Plataforma' : 'Nova Plataforma' }}</h4>
</div>
                ...
                <form action="{{ isset($dado) ? route('plataforma.update', $dado->id) : route('plataforma.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="ano_lancamento" class="form-control" min="1970" max="{{ date('Y')+1 }}"
                                value="{{ old('ano_lancamento', $dado->ano_lancamento ?? '') }}" placeholder="{{ date('Y') }}">
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Imagem</label>
                        @if(isset($dado) && $dado->imagem)
                            <div class="mb-2"><img src="/storage/{{ $dado->imagem }}" style="max-height:80px;border-radius:6px;border:1px solid var(--gv-border)" alt=""></div>
                        @endif
