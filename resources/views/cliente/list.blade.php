...
<div class="d-flex align-items-center justify-content-between mb-4">
    ...
    <a href="{{ route('cliente.create') }}" class="btn btn-gv">
...
<div class="card-gv mb-3">
    <div style="padding:1rem 1.25rem">
        <form action="{{ route('cliente.search') }}" method="POST" class="d-flex gap-2">
            ...
            <a href="{{ route('cliente.index') }}" class="btn btn-outline-gv">
                <i class="bi bi-x"></i>
            </a>
        </form>
    </div>
        ...
        <table class="table mb-0">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="font-size:0.82rem;color:var(--gv-muted)">
                        {{ $item->cidade ? $item->cidade . '/' . $item->estado : '—' }}
                    </td>
                    <td>
                        <span style="color:var(--gv-accent);font-family:'Rajdhani',sans-serif;font-weight:700">
                            {{ $item->pedidos->count() }}
                        </span>
                    </td>
                    <td>
                    ...
                    <td>
                        <a href="{{ route('cliente.edit', $item->id) }}" class="btn btn-sm btn-outline-gv">
                    ...
                    <td>
                        <form action="{{ route('cliente.destroy', $item->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-sm" style="border:1px solid var(--gv-red);color:var(--gv-red);background:none" onclick="return confirm('Remover cliente?')">
                ...
                @empty
                <tr>
                    <td colspan="10" class="text-center" style="padding:3rem;color:var(--gv-muted)">
